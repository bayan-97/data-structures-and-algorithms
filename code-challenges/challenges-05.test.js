...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(element => {
    let personClone = $('#template').first().clone();
    personClone.removeAttr('id');
   personClone.find('h2').text(element.name);
   personClone.find('h3').text(element.height);
   personClone.find('p').text(element.eye_color);
  
    $('main').append(personClone);
...
const listFoods = (recipe) => {
 ...
 result.push( element.slice(-8))
     
   }  else if (index==1||index==2||index==3) {
    result.push( element.slice(9))
   } else if (index==9||index==10||index==4){
    result.push( element.slice(10))
   } else if (index==8||index==7||index==6||index==5){
