...
const createServer = () => {
  ...
  app.get('/aboutme', (req,res)=>{
    ...
    res.status(200).send("my name is bayan i am web developer.i love coding so much ")
  })
  app.get('/favoritefoods', (req,res)=>{
    let food=["pasta","mansaf","pizza"]
...
const totalSum = (input) => {
 ...
 input.map(element=>{
   ...
   let sum = element.reduce((accumaltor, value, index)=>{
     accumaltor += value;
     return accumaltor;
...
if (element==[]) {
  ...
  let evens = element.filter((n,i)=>{
    if(typeof n  ==='string'){
      return 0}else if (n%5===0) {return n}
})
if (evens==[]) {
  res.push([])

}else{
  let neweven=evens.map((element2,inex)=>{
  
    return  Math.pow(2 ,element2)
